...
$uyeAdi = session('uye');
$resimlerim = $baglanti->query("SELECT * FROM resimler WHERE resimYukleyenId = (SELECT userId FROM uyeler WHERE userName = '{$uyeAdi}')",PDO::FETCH_ASSOC);
?>
    <div class="row">
...
<?php
        if($resimlerim->rowCount()){
            foreach ($resimlerim as $key){
                ...
                ?>
                <div class="fotoDiv">
                    <a href="<?php echo asset_url('images/'.$kategori . '/' .$key['resimYol'])?>" data-lightbox="Fotoğraf-1" data-title="Lütfen kapatmak için rastgele bir yere basın"><img class="fotoDivFoto" class="img-resposive" src="<?php echo asset_url('images/'.$kategori . '/' .$key['resimYol'])?>"></a>
